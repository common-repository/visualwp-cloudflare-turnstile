...
function vwptn_init_turnstile_verification(WP_User $user) {
	...
	if(isset($_POST['action']) && $_POST['action'] == 'elementor_woocommerce_checkout_login_user') {
		return $user;
	}
...
function vwptn_init_turnstile_verification_lostpassword(WP_Error $error) {
	if(isset($_POST['action']) && $_POST['action'] == 'elementor_woocommerce_checkout_login_user') {
		return $error;
	}
	$data = sanitize_text_field($_POST['cf-turnstile-response']);
	$response = vwptn_get_turnstile_response($data);	
	if($response != 1) {
		$error = new WP_Error( 'authentication_failed',esc_html($response));		  
	}
	return $error;
...
add_action('lostpassword_post','vwptn_init_turnstile_verification_lostpassword', 10, 1);
...
if ( in_array( 'ninja-forms/ninja-forms.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	function vwptn_ninja_forms_submit_data( $form_data ) {
	  ...
	  foreach( $form_data[ 'fields' ] as $key=>$field ) { // Field settings, including the field key and value.
	  ...
	  if(stristr($field['key'],'turnstile')) {
	   ...
	   $response = esc_html(vwptn_get_turnstile_response($field['value']));	
		...
		//		  $extra_data = $form_data[ 'extra' ]; // Extra data included with the submission.
				 $field['value'] = 'success';
				 $form_data['fields'][$key]['value'] = $response;
